...
class Web::LecturesController < Web::ApplicationController
  def index
    @top_lectures = LectureDecorator.decorate_collection Lecture.includes(:workshop).where(move_to_top: true).web.by_listener_votes.ransack(params[:q]).result.limit(configus.move_to_top_count)
