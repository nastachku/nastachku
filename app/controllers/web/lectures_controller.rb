...
class Web::LecturesController < Web::ApplicationController
  ...
  def index
    @lectures_with_lector = LectureDecorator.decorate_collection Lecture.includes(:workshop, :user).web.with_active_speaker.by_listener_votes.ransack(params).result
